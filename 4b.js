...
const Element = mongoose.model('Element1', ElementSchema);
    ...
    try {
     // console.log(`Scraping: ${element[1]} (${element[0]})`);
      ...
      if (elementHandle) {
        ...
        // Go back to the search page by clicking the logo
...
async function scrapeRecipes() {
  ...
  const browsers = await Promise.all(Array(4).fill().map(() => puppeteer.launch({ headless: 'new', args: ['--no-sandbox'] })));

  const elements = Object.entries(data).map(([key, value]) => value);
  const chunkSize = Math.ceil(elements.length / 4);
  const chunks = Array(4).fill().map((_, index) => elements.slice(index * chunkSize, (index + 1) * chunkSize));
